...
PROGRAM hist_concatenate
   ...
   integer, parameter :: compress = 1
      ...
      CALL hist_concatenate_var_3d (filehist, 'f_smin_no3_vr', timelen, 'soil', &
         nl_soil, compress, 'mineral no3 density in soil layers','gN/m3')

            ! mineral nh4 density in soil layers
      CALL hist_concatenate_var_3d (filehist, 'f_smin_nh4_vr', timelen, 'soil', &
         nl_soil, compress, 'mineral nh4 density in soil layers','gN/m3')

      IF(DEF_USE_SOILACIDIFICATION)THEN
            ! pH in soil layers
         CALL hist_concatenate_var_3d (filehist, 'f_pH_vr', timelen, 'soil', &
            nl_soil, compress, 'pH in soil layers','-')
         
         CALL hist_concatenate_var_3d (filehist, 'f_pH_init_vr', timelen, 'soil', &
            nl_soil, compress, 'Init pH in soil layers','-')

         ! CALL hist_concatenate_var_3d (filehist, 'f_calcite_mass', timelen, 'soil',  &
         !    nl_soil, compress, 'calcite_mass in soil layers','kg/m2')

         CALL hist_concatenate_var_3d (filehist, 'f_calcite_content', timelen, 'soil',  &
            nl_soil, compress, 'calcite_content in soil layers','%')

         CALL hist_concatenate_var_3d ( filehist, 'f_h_from_N', timelen, 'soil',  &
            nl_soil, compress, 'h_from_N in soil layers','mol/l/s')

         CALL hist_concatenate_var_3d (filehist, 'f_rate_cal', timelen, 'soil',  &
            nl_soil, compress, 'rate_cal in soil layers','mol/l/s')

         CALL hist_concatenate_var_3d ( filehist, 'f_rate_CO2', timelen, 'soil',  &
            nl_soil, compress, 'rate_CO2 in soil layers','mol/l/s')

         CALL hist_concatenate_var_3d ( filehist, 'f_rate_Bcex', timelen, 'soil',  &
            nl_soil, compress, 'rate_Bcex in soil layers','mol/l/s')

         CALL hist_concatenate_var_3d ( filehist, 'f_rate_Hex', timelen, 'soil',  &
            nl_soil, compress, 'rate_Hex in soil layers','mol/l/s')

         CALL hist_concatenate_var_3d ( filehist, 'f_pCO2_soil', timelen, 'soil',  &
            nl_soil, compress, 'pCO2_soil in soil layers','ppmv')

         CALL hist_concatenate_var_3d ( filehist, 'f_E_Bc', timelen, 'soil',  &
            nl_soil, compress, 'E_Bc in soil layers','%')
         
         CALL hist_concatenate_var_3d ( filehist, 'f_CEC', timelen, 'soil',  &
            nl_soil, compress, 'CEC in soil layers','cmol(+)/kg')

         CALL hist_concatenate_var_3d ( filehist, 'f_h_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'h_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_ca_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'ca_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_DIC_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'DIC_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_ANC_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'ANC_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_Bc_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'Bc_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_Bct_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'Bct_conc_vr in soil layers','mol/l')

         CALL hist_concatenate_var_3d ( filehist, 'f_al3_conc_vr', timelen, 'soil',  &
            nl_soil, compress, 'al3_conc_vr in soil layers','mol/l')
