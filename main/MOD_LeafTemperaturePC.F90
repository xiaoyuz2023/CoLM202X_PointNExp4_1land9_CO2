...
! input variables
   ...
   real(r8), intent(in) :: &
        pH_vr(1:nl_soil,ps:pe), &  ! soil pH in each layer
        rootr(1:nl_soil,ps:pe)! water uptake fraction from different layers for each PFT
...
!-----------------------------------------------------------------------
               CALL stomata (nl_soil, vmax25(i)    ,effcon(i) ,c3c4(i)   ,slti(i)   ,hlti(i)   ,&
...
!WUE stomata model parameter
               ...
               IF (DEF_USE_PLANTHYDRAULICS) THEN
                  ...
                  CALL update_photosyn(nl_soil, tl(i), po2m, pco2m, pco2a, parsun(i), psrf, rstfacsun(i), &
                     rb(i), gssun(i), effcon(i), vmax25(i), c3c4(i), gradm(i), trop(i), slti(i), hlti(i), &
                     shti(i), hhti(i), trda(i), trdm(i), cintsun(:,i), pH_vr(:,i), rootr(:,i), assimsun(i), respcsun(i))

                  CALL update_photosyn(nl_soil, tl(i), po2m, pco2m, pco2a, parsha(i), psrf, rstfacsha(i), &
                     rb(i), gssha(i), effcon(i), vmax25(i), c3c4(i), gradm(i), trop(i), slti(i), hlti(i), &
                     shti(i), hhti(i), trda(i), trdm(i), cintsha(:,i), pH_vr(:,i), rootr(:,i), assimsha(i), respcsha(i))
