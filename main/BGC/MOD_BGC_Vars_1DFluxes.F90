...
! ecosystem vegetation carbon/nitrogen flux
   ...
   real(r8), allocatable :: rr                         (:) ! root respiration (fine root MR + total root GR) (gC m-2 s-1)
   ...
   real(r8), allocatable :: soil_er                    (:) ! soil respiration (gC m-2 s-1)
   real(r8), allocatable :: soil_er_vr                (:,:) ! vertical resolved soil respiration (gC m-3 s-1)
 ...
 ! vegetation to decomposition nitrogen fluxes
   ...
   real(r8), allocatable :: h_content_cons_vr        (:,:)   ! vertical resolved: H+ consumption (mol m-2 s-1)
   real(r8), allocatable :: h_content_prod_vr        (:,:)   ! vertical resolved: H+ production (mol m-2 s-1)
...
! ecosystem carbon flux
            ...
            allocate (soil_er                    (numpatch)) ; soil_er                    (:) = spval
            allocate (soil_er_vr                (nl_soil_full,numpatch)); soil_er_vr      (:,:) = spval
...
! vegetation to decomposition nitrogen fluxes
            ...
            allocate (h_content_cons_vr        (nl_soil,numpatch)); h_content_cons_vr        (:,:) = spval
            allocate (h_content_prod_vr        (nl_soil,numpatch)); h_content_prod_vr        (:,:) = spval
...
! ecosystem carbon flux
 ...
 ! vegetation to decomposition nitrogen fluxes
            ...
            deallocate (h_content_cons_vr        )
            deallocate (h_content_prod_vr        )
...
! vegetation to decomposition nitrogen fluxes
            ...
            h_content_cons_vr        (:,:)   = Values
            h_content_prod_vr        (:,:)   = Values
