...
! Xingjie Lu, 2021, revised the CLM5 code to be compatible with CoLM code structure.
   ...
   USE MOD_BGC_Vars_1DPFTFluxes, only: &
         leaf_mr_p, froot_mr_p, froot_mr_vr_p, livestem_mr_p, livecroot_mr_p, grain_mr_p
...
CONTAINS
   ...
   REAL(r8):: pH_opt, pH_sens, pH_factor_p(nl_soil)
...
! column loop to calculate temperature factors in each soil layer
      ...
      ! Calculate pH factors for each soil layer (outside pft loop)
      IF(DEF_USE_SOILACIDIFICATION .and. DEF_SOILACID_FACTOR_ER) THEN
         ...
         DO j = 1, nl_soil
            ...
            pH_factor_p(j) = EXP(-((pH_vr(j,i) - pH_opt) / pH_sens)**2)
            write(*,*) 'Soil RR_M : pH =', pH_vr(j,i), ' pH_factor_p =', pH_factor_p(j), j
         ENDDO
      ELSE
         DO j = 1, nl_soil
            pH_factor_p(j) = 1.0_r8
         ENDDO
      ENDIF
      ...
      DO m = ps, pe
         ...
         ! function of temperature and N content.
            froot_mr_vr_p(j,m) = frootn_p(m)*br_root*tcsoi(j)*rootfr_p(j,ivt)*pH_factor_p(j)
            froot_mr_p(m) = froot_mr_p(m) + froot_mr_vr_p(j,m)
            ! write(*,*) 'ER-Debug layer =', j, ' froot_mr_p =', froot_mr_p(m), ' frootn_p**=', frootn_p(m)*br_root*tcsoi(j)*rootfr_p(j,ivt)*pH_factor_p(j), frootn_p(m), br_root, tcsoi(j), rootfr_p(j,ivt), pH_factor_p(j)
         ENDDO
      ENDDO
