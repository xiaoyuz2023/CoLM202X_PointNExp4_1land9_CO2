...
! Xingjie Lu, 2021, revised original CLM5 code to be compatible with CoLM code structure.
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_NITRIF, DEF_SOILACID_FACTOR_ER, DEF_USE_SOILACIDIFICATION, &
   ...
   USE MOD_Vars_TimeVariables, only: t_scalar, w_scalar
...
CONTAINS
   ...
   real(r8) :: env_factor       ! combined environmental factor (pH * T * W)
      ...
      DO k = 1, ndecomp_transitions
         DO j = 1,nl_soil
            ...
            IF(DEF_USE_SOILACIDIFICATION .and. DEF_SOILACID_FACTOR_ER) THEN
               ...
               pH_factor = EXP(-((pH_vr(j,i) - pH_opt) / pH_sens)**2)
               env_factor = pH_factor * t_scalar(j,i) * w_scalar(j,i)
               
               ! write(*,*) 'Soil decomp_hr layer, pH =', j, pH_vr(j,i), ' pH_factor =', pH_factor , &
               !          ' T_scalar =', t_scalar(j,i), ' W_scalar =', w_scalar(j,i)
            ENDIF
               decomp_hr_vr(j,k,i) = rf_decomp(j,k,i) * p_decomp_cpool_loss(j,k,i) * env_factor
               ! write(*,*) 'ER-Debug layer =', j, ' decomp_hr_vr =', decomp_hr_vr(j,k,i), ' rf_decomp =', rf_decomp(j,k,i), ' p_decomp_cpool_loss =', p_decomp_cpool_loss(j,k,i), ' env_factor =', env_factor
