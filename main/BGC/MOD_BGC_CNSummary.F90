...
! Xingjie Lu, 2022, modify original CLM5 to be compatible with CoLM code structure. 
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_NITRIF, DEF_USE_DiagMatrix, DEF_USE_SOILACIDIFICATION, DEF_SOILACID_FACTOR_ER
   ...
   USE MOD_BGC_Vars_TimeVariables, only: &
       ...
       totvegc, totvegn, totcolc, totcoln, sminn, sminn_vr, totsoiln_vr, pH_vr, &
   ...
   USE MOD_BGC_Vars_1DFluxes, only: &
      ...
      decomp_hr, decomp_hr_vr, gpp, ar, rr, er, soil_er, soil_er_vr, supplement_to_sminn, supplement_to_sminn_vr, &
#ifdef CROP
   ...
   USE MOD_BGC_Vars_1DPFTFluxes, only: &
         psn_to_cpool_p, leaf_mr_p, froot_mr_p, froot_mr_vr_p, livestem_mr_p, livecroot_mr_p, &
       ...
       fire_closs_p, hrv_xsmrpool_to_atm_p, rr_p, &
#ifdef CROP
...
! Xingjie Lu, 2022, modify original CLM5 to be compatible with CoLM code structure. 
   ...
   real(r8),intent(in) :: deltim               ! time step in seconds
      ...
      CALL cnveg_carbonflux_summary(i,nl_soil,dz_soi,ps,pe,ndecomp_transitions,deltim)
...
! Xingjie Lu, 2022, modify original CLM5 to be compatible with CoLM code structure. 
   ...
   SUBROUTINE cnveg_carbonflux_summary(i,nl_soil,dz_soi,ps,pe,ndecomp_transitions,deltim)
...
! Xingjie Lu, 2022, modify original CLM5 to be compatible with CoLM code structure. 
   ...
   integer, intent(in) :: ps      ! start pft index
   integer, intent(in) :: pe      ! END pft index
   integer, intent(in) :: ndecomp_transitions ! number of decomposition transitions
   real(r8),intent(in) :: deltim  ! time step in seconds

   integer m, j, k
   real(r8) nfixlags
   real(r8) :: root_gr_resp_p     ! pft-level root growth respiration
   real(r8) :: soil_pH_root     ! root-zone weighted average pH
   real(r8) :: root_weight_sum  ! sum of root fractions
   real(r8) :: pH_factor        ! pH effect on soil respiration (Gaussian function)
   real(r8) :: pH_opt           ! optimal pH for soil respiration
   real(r8) :: pH_sens          ! pH sensitivity parameter
   ...
   soil_er_vr(:,i) = 0._r8
   DO j = 1, nl_soil
      ! Heterotrophic respiration is already volumetric.
      DO k = 1, ndecomp_transitions
         soil_er_vr(j,i) = soil_er_vr(j,i) + decomp_hr_vr(j,k,i)
      ENDDO

      ! Root respiration terms are converted from layer areal flux to volumetric flux.
      DO m = ps, pe
         IF (dz_soi(j) > 0._r8) THEN
            root_gr_resp_p = cpool_froot_gr_p(m) + cpool_livecroot_gr_p(m) + cpool_deadcroot_gr_p(m) &
                           + transfer_froot_gr_p(m) + transfer_livecroot_gr_p(m) + transfer_deadcroot_gr_p(m) &
                           + cpool_froot_storage_gr_p(m) + cpool_livecroot_storage_gr_p(m) + cpool_deadcroot_storage_gr_p(m)
            soil_er_vr(j,i) = soil_er_vr(j,i) + froot_mr_vr_p(j,m) * pftfrac(m) / dz_soi(j) &
                            + root_gr_resp_p * pftfrac(m) * rootfr_p(j,pftclass(m)) / dz_soi(j)
         ENDIF
      ENDDO
      
   ENDDO

   soil_er(i) = sum(soil_er_vr(1:nl_soil,i) * dz_soi(1:nl_soil))
