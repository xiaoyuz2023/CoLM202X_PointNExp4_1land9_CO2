...
! Xingjie Lu, 2022, modify original CLM5 to be compatible with CoLM code structure.
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_SASU, DEF_USE_DiagMatrix, DEF_USE_NITRIF, DEF_USE_CNSOYFIXN, DEF_USE_FIRE, DEF_USE_IRRIGATION, DEF_USE_SOILACIDIFICATION
   ...
   USE MOD_BGC_Soil_BiogeochemNStateUpdate1, only: SoilBiogeochemNStateUpdate1, soil_acidification_buffering
   ...
   integer j
...
#endif
      CALL CNGResp(i, ps, pe, nl_soil, npcropmin)
#ifdef CROP
      ...
      IF(DEF_USE_SOILACIDIFICATION)THEN
         CALL soil_acidification_buffering(i,ps,pe,deltim,nl_soil,dz_soi,z_soi)
      ENDIF
