...
! Xingjie Lu, 2021, revised original CLM5 code to be compatible with CoLM code structure.
   ...
   USE MOD_Precision
   USE MOD_Namelist, only: DEF_USE_SOILACIDIFICATION, DEF_SOILACID_FACTOR_NITR
   ...
   USE MOD_BGC_Vars_TimeVariables, only: &
       ...
       to2_decomp_depth_unsat, tconc_o2_unsat, smin_nh4_vr, smin_no3_vr, pH_vr
...
CONTAINS
   ...
   real(r8),parameter :: PI = 4.*atan(1.)
      ...
      DO j = 1, nl_soil
         ...
         IF (DEF_USE_SOILACIDIFICATION .and. DEF_SOILACID_FACTOR_NITR) THEN
            IF (pH_vr(j,i) <= 0._r8) THEN
               pH = 6.5_r8
            ELSE
               pH = pH_vr(j,i)
            ENDIF
         ...
         ENDIF
            ...
            ! write(*,*) 'Nitr : soil_pH_root =', pH, ' pH_factor =', k_nitr_ph_vr, ' layer ', j
#endif
