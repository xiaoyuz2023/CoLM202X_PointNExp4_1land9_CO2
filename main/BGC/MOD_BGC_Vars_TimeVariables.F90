...
!------------------------- BGC variables -------------------------------
   ...
   real(r8), allocatable :: pCO2_hr           (:,:)           ! vertical resolved: the increase of soil CO2 partial pressure (ppmv)
   real(r8), allocatable :: pH_vr             (:,:)           ! vertical resolved: soil pH
   real(r8), allocatable :: pH_init_vr        (:,:)           ! vertical resolved: soil pH at the beginning of time step
   real(r8), allocatable :: h_conc_vr         (:,:)           ! vertical resolved: soil H+ concentration (mol/l) 
   real(r8), allocatable :: h_conc_m1         (:,:)           ! vertical resolved: soil H+ concentration at previous time step (mol/l)
   real(r8), allocatable :: oh_conc_vr        (:,:)
   real(r8), allocatable :: oh_conc_m1        (:,:)           ! vertical resolved: soil OH- concentration at previous time step (mol/l)
   real(r8), allocatable :: ca_conc_vr        (:,:)           ! vertical resolved: soil Ca2+ concentration (mol/l)
   real(r8), allocatable :: ca_conc_m1        (:,:)           ! vertical resolved: soil Ca2+ concentration at previous time step (mol/l)
   real(r8), allocatable :: Bc_conc_vr        (:,:)
   real(r8), allocatable :: Bc_conc_m1        (:,:)           ! vertical resolved: soil Bc2+ concentration at previous time step (mol/l)
   real(r8), allocatable :: Bct_conc_vr       (:,:)
   real(r8), allocatable :: al3_conc_vr       (:,:)           ! vertical resolved: soil Al3+ concentration (mol/l)
   real(r8), allocatable :: al2_conc_vr       (:,:)  
   real(r8), allocatable :: al_conc_vr        (:,:)      
   real(r8), allocatable :: nh4_conc_vr       (:,:)
   real(r8), allocatable :: no3_conc_vr       (:,:)
   real(r8), allocatable :: DIC_conc_vr       (:,:)
   real(r8), allocatable :: DIC_conc_m1       (:,:)           ! vertical resolved: soil DIC concentration at previous time step (mol/l)
   real(r8), allocatable :: h2co3_conc_vr     (:,:)
   real(r8), allocatable :: hco3_conc_vr      (:,:)
   real(r8), allocatable :: co3_conc_vr       (:,:)
   real(r8), allocatable :: ANC_conc_vr       (:,:)           ! vertical resolved: soil ANC concentration (mol/l)
   real(r8), allocatable :: rate_cal          (:,:)
   real(r8), allocatable :: rate_CO2          (:,:)
   real(r8), allocatable :: rate_Bcex         (:,:)
   real(r8), allocatable :: rate_Hex          (:,:)
   real(r8), allocatable :: pCO2_soil         (:,:)
   real(r8), allocatable :: h_from_N          (:,:)           ! mol/l/s
   real(r8), allocatable :: E_Bc              (:,:)
   real(r8), allocatable :: calcite_mass      (:,:)           ! kg/m2
   real(r8), allocatable :: calcite_content   (:,:)
...
! ------------------------------------------------------
   ...
   IMPLICIT NONE
    ...
    ! bgc variables
            ...
            allocate (pCO2_hr                      (nl_soil,numpatch))                    ; pCO2_hr             (:,:) = spval
            allocate (pH_vr                        (nl_soil,numpatch))                    ; pH_vr               (:,:) = spval
            allocate (pH_init_vr                   (nl_soil,numpatch))                    ; pH_init_vr          (:,:) = spval
            allocate (h_conc_vr                    (nl_soil,numpatch))                    ; h_conc_vr           (:,:) = spval
            allocate (h_conc_m1                    (nl_soil,numpatch))                    ; h_conc_m1           (:,:) = spval
            allocate (oh_conc_vr                   (nl_soil,numpatch))                    ; oh_conc_vr          (:,:) = spval
            allocate (oh_conc_m1                   (nl_soil,numpatch))                    ; oh_conc_m1          (:,:) = spval
            allocate (ca_conc_vr                   (nl_soil,numpatch))                    ; ca_conc_vr          (:,:) = spval
            allocate (ca_conc_m1                   (nl_soil,numpatch))                    ; ca_conc_m1          (:,:) = spval
            allocate (Bc_conc_vr                   (nl_soil,numpatch))                    ; Bc_conc_vr          (:,:) = spval
            allocate (Bc_conc_m1                   (nl_soil,numpatch))                    ; Bc_conc_m1          (:,:) = spval
            allocate (Bct_conc_vr                  (nl_soil,numpatch))                    ; Bct_conc_vr         (:,:) = spval
            allocate (al3_conc_vr                 (nl_soil,numpatch))                    ; al3_conc_vr        (:,:) = spval
            allocate (al2_conc_vr                 (nl_soil,numpatch))                    ; al2_conc_vr        (:,:) = spval
            allocate (al_conc_vr                  (nl_soil,numpatch))                    ; al_conc_vr         (:,:) = spval
            allocate (nh4_conc_vr                  (nl_soil,numpatch))                    ; nh4_conc_vr         (:,:) = spval
            allocate (no3_conc_vr                  (nl_soil,numpatch))                    ; no3_conc_vr         (:,:) = spval
            allocate (DIC_conc_vr                  (nl_soil,numpatch))                    ; DIC_conc_vr         (:,:) = spval
            allocate (DIC_conc_m1                  (nl_soil,numpatch))                    ; DIC_conc_m1         (:,:) = spval
            allocate (h2co3_conc_vr                (nl_soil,numpatch))                    ; h2co3_conc_vr       (:,:) = spval
            allocate (hco3_conc_vr                 (nl_soil,numpatch))                    ; hco3_conc_vr        (:,:) = spval
            allocate (co3_conc_vr                  (nl_soil,numpatch))                    ; co3_conc_vr         (:,:) = spval
            allocate (ANC_conc_vr                  (nl_soil,numpatch))                    ; ANC_conc_vr         (:,:) = spval
            allocate (rate_cal                     (nl_soil,numpatch))                    ; rate_cal            (:,:) = spval
            allocate (rate_CO2                     (nl_soil,numpatch))                    ; rate_CO2            (:,:) = spval
            allocate (rate_Bcex                    (nl_soil,numpatch))                    ; rate_Bcex           (:,:) = spval
            allocate (rate_Hex                     (nl_soil,numpatch))                    ; rate_Hex            (:,:) = spval
            allocate (pCO2_soil                    (nl_soil,numpatch))                    ; pCO2_soil           (:,:) = spval
            allocate (h_from_N                     (nl_soil,numpatch))                    ; h_from_N            (:,:) = spval
            allocate (E_Bc                         (nl_soil,numpatch))                    ; E_Bc                (:,:) = spval
            allocate (calcite_mass                 (nl_soil,numpatch))                    ; calcite_mass        (:,:) = spval
            allocate (calcite_content              (nl_soil,numpatch))                    ; calcite_content     (:,:) = spval
            allocate (CEC                          (nl_soil,numpatch))                    ; CEC                 (:,:) = spval
...
! --------------------------------------------------
 ...
 ! bgc variables
            ...
            deallocate (pCO2_hr                      )
            deallocate (pH_vr                        )
            deallocate (pH_init_vr                   )
            deallocate (h_conc_vr                    )
            deallocate (h_conc_m1                    )
            deallocate (oh_conc_vr                   )
            deallocate (oh_conc_m1                   )
            deallocate (ca_conc_vr                   )
            deallocate (ca_conc_m1                   )
            deallocate (Bc_conc_vr                   )
            deallocate (Bc_conc_m1                   )
            deallocate (Bct_conc_vr                  )
            deallocate (al3_conc_vr                 )
            deallocate (al2_conc_vr                 )
            deallocate (al_conc_vr                  )
            deallocate (nh4_conc_vr                  )
            deallocate (no3_conc_vr                  )
            deallocate (DIC_conc_vr                  )
            deallocate (DIC_conc_m1                  )
            deallocate (h2co3_conc_vr                )
            deallocate (hco3_conc_vr                 )
            deallocate (co3_conc_vr                  )
            deallocate (ANC_conc_vr                  )
            deallocate (rate_cal                     )
            deallocate (rate_CO2                     )
            deallocate (rate_Bcex                    )
            deallocate (rate_Hex                     )
            deallocate (pCO2_soil                    )
            deallocate (h_from_N                     )
            deallocate (E_Bc                         )
            deallocate (calcite_mass                 )
            deallocate (calcite_content              )
            deallocate (CEC                          )
...
!=======================================================================
 ...
 ! bgc variables
      ...
      CALL ncio_write_vector (file_restart, 'pCO2_hr              ', 'soil' ,   nl_soil, 'patch', landpatch, pCO2_hr        )                                                                
      CALL ncio_write_vector (file_restart, 'pH_vr                ', 'soil' ,   nl_soil, 'patch', landpatch, pH_vr          )
      CALL ncio_write_vector (file_restart, 'pH_init_vr           ', 'soil' ,   nl_soil, 'patch', landpatch, pH_init_vr     )
      CALL ncio_write_vector (file_restart, 'h_conc_vr            ', 'soil' ,   nl_soil, 'patch', landpatch, h_conc_vr      )
      CALL ncio_write_vector (file_restart, 'h_conc_m1            ', 'soil' ,   nl_soil, 'patch', landpatch, h_conc_m1      )
      CALL ncio_write_vector (file_restart, 'oh_conc_vr           ', 'soil' ,   nl_soil, 'patch', landpatch, oh_conc_vr     )
      CALL ncio_write_vector (file_restart, 'oh_conc_m1           ', 'soil' ,   nl_soil, 'patch', landpatch, oh_conc_m1     )
      CALL ncio_write_vector (file_restart, 'ca_conc_vr           ', 'soil' ,   nl_soil, 'patch', landpatch, ca_conc_vr     )
      CALL ncio_write_vector (file_restart, 'ca_conc_m1           ', 'soil' ,   nl_soil, 'patch', landpatch, ca_conc_m1     )
      CALL ncio_write_vector (file_restart, 'Bc_conc_vr           ', 'soil' ,   nl_soil, 'patch', landpatch, Bc_conc_vr     )
      CALL ncio_write_vector (file_restart, 'Bc_conc_m1           ', 'soil' ,   nl_soil, 'patch', landpatch, Bc_conc_m1     )
      CALL ncio_write_vector (file_restart, 'Bct_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, Bct_conc_vr     )
      CALL ncio_write_vector (file_restart, 'al3_conc_vr         ', 'soil' ,   nl_soil, 'patch', landpatch, al3_conc_vr   )
      CALL ncio_write_vector (file_restart, 'al2_conc_vr         ', 'soil' ,   nl_soil, 'patch', landpatch, al2_conc_vr   )
      CALL ncio_write_vector (file_restart, 'al_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, al_conc_vr    )
      CALL ncio_write_vector (file_restart, 'nh4_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, nh4_conc_vr    )
      CALL ncio_write_vector (file_restart, 'no3_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, no3_conc_vr    )
      CALL ncio_write_vector (file_restart, 'DIC_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, DIC_conc_vr    )
      CALL ncio_write_vector (file_restart, 'DIC_conc_m1          ', 'soil' ,   nl_soil, 'patch', landpatch, DIC_conc_m1    )
      CALL ncio_write_vector (file_restart, 'h2co3_conc_vr        ', 'soil' ,   nl_soil, 'patch', landpatch, h2co3_conc_vr  )
      CALL ncio_write_vector (file_restart, 'hco3_conc_vr         ', 'soil' ,   nl_soil, 'patch', landpatch, hco3_conc_vr   )
      CALL ncio_write_vector (file_restart, 'co3_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, co3_conc_vr    )
      CALL ncio_write_vector (file_restart, 'ANC_conc_vr          ', 'soil' ,   nl_soil, 'patch', landpatch, ANC_conc_vr    )
      CALL ncio_write_vector (file_restart, 'rate_cal             ', 'soil' ,   nl_soil, 'patch', landpatch, rate_cal       )
      CALL ncio_write_vector (file_restart, 'rate_CO2             ', 'soil' ,   nl_soil, 'patch', landpatch, rate_CO2       )
      CALL ncio_write_vector (file_restart, 'rate_Bcex            ', 'soil' ,   nl_soil, 'patch', landpatch, rate_Bcex      )
      CALL ncio_write_vector (file_restart, 'rate_Hex             ', 'soil' ,   nl_soil, 'patch', landpatch, rate_Hex       )
      CALL ncio_write_vector (file_restart, 'pCO2_soil            ', 'soil' ,   nl_soil, 'patch', landpatch, pCO2_soil      )
      CALL ncio_write_vector (file_restart, 'h_from_N             ', 'soil' ,   nl_soil, 'patch', landpatch, h_from_N       )
      CALL ncio_write_vector (file_restart, 'E_Bc                 ', 'soil' ,   nl_soil, 'patch', landpatch, E_Bc           )
      CALL ncio_write_vector (file_restart, 'calcite_mass         ', 'soil' ,   nl_soil, 'patch', landpatch, calcite_mass   )
      CALL ncio_write_vector (file_restart, 'calcite_content      ', 'soil' ,   nl_soil, 'patch', landpatch, calcite_content)
      CALL ncio_write_vector (file_restart, 'CEC                  ', 'soil' ,   nl_soil, 'patch', landpatch, CEC            )
...
! bgc variables
      ...
      CALL ncio_read_vector (file_restart, 'pCO2_hr              ',   nl_soil, landpatch, pCO2_hr            )
      CALL ncio_read_vector (file_restart, 'pH_vr                ',   nl_soil, landpatch, pH_vr              )
      CALL ncio_read_vector (file_restart, 'pH_init_vr           ',   nl_soil, landpatch, pH_init_vr         )
      CALL ncio_read_vector (file_restart, 'h_conc_vr            ',   nl_soil, landpatch, h_conc_vr          )
      CALL ncio_read_vector (file_restart, 'h_conc_m1            ',   nl_soil, landpatch, h_conc_m1          )
      CALL ncio_read_vector (file_restart, 'oh_conc_vr           ',   nl_soil, landpatch, oh_conc_vr         )
      CALL ncio_read_vector (file_restart, 'oh_conc_m1           ',   nl_soil, landpatch, oh_conc_m1         )
      CALL ncio_read_vector (file_restart, 'ca_conc_vr           ',   nl_soil, landpatch, ca_conc_vr         )
      CALL ncio_read_vector (file_restart, 'ca_conc_m1           ',   nl_soil, landpatch, ca_conc_m1         )
      CALL ncio_read_vector (file_restart, 'Bc_conc_vr           ',   nl_soil, landpatch, Bc_conc_vr         )
      CALL ncio_read_vector (file_restart, 'Bc_conc_m1           ',   nl_soil, landpatch, Bc_conc_m1         )
      CALL ncio_read_vector (file_restart, 'Bct_conc_vr          ',   nl_soil, landpatch, Bct_conc_vr        )
      CALL ncio_read_vector (file_restart, 'al3_conc_vr         ',   nl_soil, landpatch, al3_conc_vr       )
      CALL ncio_read_vector (file_restart, 'al2_conc_vr         ',   nl_soil, landpatch, al2_conc_vr       )
      CALL ncio_read_vector (file_restart, 'al_conc_vr          ',   nl_soil, landpatch, al_conc_vr        )
      CALL ncio_read_vector (file_restart, 'nh4_conc_vr          ',   nl_soil, landpatch, nh4_conc_vr        )
      CALL ncio_read_vector (file_restart, 'no3_conc_vr          ',   nl_soil, landpatch, no3_conc_vr        )
      CALL ncio_read_vector (file_restart, 'DIC_conc_vr          ',   nl_soil, landpatch, DIC_conc_vr        )
      CALL ncio_read_vector (file_restart, 'DIC_conc_m1          ',   nl_soil, landpatch, DIC_conc_m1        )
      CALL ncio_read_vector (file_restart, 'h2co3_conc_vr        ',   nl_soil, landpatch, h2co3_conc_vr      )
      CALL ncio_read_vector (file_restart, 'hco3_conc_vr         ',   nl_soil, landpatch, hco3_conc_vr       )
      CALL ncio_read_vector (file_restart, 'co3_conc_vr          ',   nl_soil, landpatch, co3_conc_vr        )
      CALL ncio_read_vector (file_restart, 'ANC_conc_vr          ',   nl_soil, landpatch, ANC_conc_vr        )
      CALL ncio_read_vector (file_restart, 'rate_cal             ',   nl_soil, landpatch, rate_cal           )
      CALL ncio_read_vector (file_restart, 'rate_CO2             ',   nl_soil, landpatch, rate_CO2           )
      CALL ncio_read_vector (file_restart, 'rate_Bcex            ',   nl_soil, landpatch, rate_Bcex          )
      CALL ncio_read_vector (file_restart, 'rate_Hex             ',   nl_soil, landpatch, rate_Hex           )
      CALL ncio_read_vector (file_restart, 'pCO2_soil            ',   nl_soil, landpatch, pCO2_soil          )
      CALL ncio_read_vector (file_restart, 'h_from_N             ',   nl_soil, landpatch, h_from_N           )
      CALL ncio_read_vector (file_restart, 'E_Bc                 ',   nl_soil, landpatch, E_Bc               )
      CALL ncio_read_vector (file_restart, 'calcite_mass         ',   nl_soil, landpatch, calcite_mass       )
      CALL ncio_read_vector (file_restart, 'calcite_content      ',   nl_soil, landpatch, calcite_content    )
      CALL ncio_read_vector (file_restart, 'CEC                  ',   nl_soil, landpatch, CEC                )
...
! bgc variables
      ...
      CALL check_vector_data ('pCO2_hr           ', pCO2_hr           )
      CALL check_vector_data ('pH_vr             ', pH_vr             )
      CALL check_vector_data ('pH_init_vr        ', pH_init_vr        )
      CALL check_vector_data ('h_conc_vr         ', h_conc_vr         )
      CALL check_vector_data ('h_conc_m1         ', h_conc_m1         )
      CALL check_vector_data ('oh_conc_vr        ', oh_conc_vr        )
      CALL check_vector_data ('oh_conc_m1        ', oh_conc_m1        )
      CALL check_vector_data ('ca_conc_vr        ', ca_conc_vr        )
      CALL check_vector_data ('ca_conc_m1        ', ca_conc_m1        )
      CALL check_vector_data ('Bc_conc_vr        ', Bc_conc_vr        )
      CALL check_vector_data ('Bc_conc_m1        ', Bc_conc_m1        )
      CALL check_vector_data ('Bct_conc_vr       ', Bct_conc_vr       )
      CALL check_vector_data ('al3_conc_vr      ', al3_conc_vr      )
      CALL check_vector_data ('al2_conc_vr      ', al2_conc_vr      )
      CALL check_vector_data ('al_conc_vr       ', al_conc_vr       )
      CALL check_vector_data ('nh4_conc_vr       ', nh4_conc_vr       )
      CALL check_vector_data ('no3_conc_vr       ', no3_conc_vr       )
      CALL check_vector_data ('DIC_conc_vr       ', DIC_conc_vr       )
      CALL check_vector_data ('DIC_conc_m1       ', DIC_conc_m1       )
      CALL check_vector_data ('h2co3_conc_vr     ', h2co3_conc_vr     )
      CALL check_vector_data ('hco3_conc_vr      ', hco3_conc_vr      )
      CALL check_vector_data ('co3_conc_vr       ', co3_conc_vr       )
      CALL check_vector_data ('ANC_conc_vr       ', ANC_conc_vr       )
      CALL check_vector_data ('rate_cal          ', rate_cal          )
      CALL check_vector_data ('rate_CO2          ', rate_CO2          )
      CALL check_vector_data ('rate_Bcex         ', rate_Bcex         )
      CALL check_vector_data ('rate_Hex          ', rate_Hex          )
      CALL check_vector_data ('pCO2_soil         ', pCO2_soil         )
      CALL check_vector_data ('h_from_N          ', h_from_N          )
      CALL check_vector_data ('E_Bc              ', E_Bc              )
      CALL check_vector_data ('calcite_mass      ', calcite_mass      )
      CALL check_vector_data ('calcite_content   ', calcite_content   )
      CALL check_vector_data ('CEC               ', CEC               )
