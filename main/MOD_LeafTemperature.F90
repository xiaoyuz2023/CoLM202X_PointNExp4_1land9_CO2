...
! input variables
   ...
   real(r8), intent(in) :: &
        ...
        pH_vr   (1:nl_soil), &! soil pH in each layer
...
!-----------------------------------------------------------------------
         ...
         IF(lai .gt. 0.001) THEN
            ...
            ! Sunlit leaves
            CALL stomata  (nl_soil, vmax25   ,effcon   ,c3c4     ,slti     ,hlti     ,&
            ...
            IF (DEF_USE_PLANTHYDRAULICS) THEN
               ...
               CALL update_photosyn(nl_soil, tl, po2m, pco2m, pco2a, parsun, psrf, rstfacsun, rb, gssun, &
                                    effcon, vmax25, c3c4, gradm, trop, slti, hlti, shti, hhti, trda, &
                                    trdm, cintsun, pH_vr, rootr, assimsun, respcsun)

               CALL update_photosyn(nl_soil, tl, po2m, pco2m, pco2a, parsha, psrf, rstfacsha, rb, gssha, &
                                    effcon, vmax25, c3c4, gradm, trop, slti, hlti, shti, hhti, trda, &
                                    trdm, cintsha, pH_vr, rootr, assimsha, respcsha)
