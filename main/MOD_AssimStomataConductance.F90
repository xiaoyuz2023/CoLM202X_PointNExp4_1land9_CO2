...
!-----------------------------------------------------------------------
   ...
   SUBROUTINE stomata (nl_soil,vmax25,effcon,c3c4,slti,hlti,shti, &
...
!Soil acidification parameter
                       pH_vr, rootr, &
!End soil acidification parameter
...
!-----------------------------------------------------------------------
      ...
      CALL calc_photo_params(nl_soil, tlef, po2m, par , psrf, rstfac, rb, effcon, vmax25, c3c4, &
                             trop, slti, hlti, shti, hhti, trda, trdm, cint, pH_vr, rootr, &
...
200   continue
   ...
   SUBROUTINE calc_photo_params(nl_soil, tlef, po2m, par , psrf, rstfac, rb, effcon, vmax25, c3c4, &
                               trop, slti, hlti, shti, hhti, trda, trdm, cint, pH_vr, rootr, &
                               vm, epar, respc, omss, gbh2o, gammas, rrkk, c3, c4)
...
!-------------------------- Dummy Arguments ----------------------------
   ...
   real(r8),intent(in) :: &
            pH_vr(nl_soil),    &! soil pH value (root-zone weighted average)
            rootr(nl_soil)     ! root fraction in each soil layer
...
!-------------------------- Local Variables ----------------------------
    real(r8) :: &
            ...
            tprcor     ! coefficient for unit transfer
   real(r8) :: &
            soil_pH_root,  &! root-zone weighted average soil pH
            root_weight_sum, &! sum of root weights for pH calculation
            pH_factor,     &! pH correction factor for soil acidification
            pH_ref! reference pH value (6.5)
   integer :: j
...
!-----------------------------------------------------------------------
   ...
   IF(DEF_USE_SOILACIDIFICATION .and. DEF_SOILACID_FACTOR_VC) THEN
      soil_pH_root = 0.0
      root_weight_sum = 0.0
      DO j = 1, nl_soil
         IF(rootr(j) > 0.0) THEN
            soil_pH_root = soil_pH_root + pH_vr(j) * rootr(j)
            root_weight_sum = root_weight_sum + rootr(j)
         ENDIF
         ! write(*,*) 'Vcmax : Soil layer', j, ' pH =', pH_vr(j), ' root fraction =', rootr(j)
      ENDDO
      
      IF(root_weight_sum > 0.0) THEN
         soil_pH_root = soil_pH_root / root_weight_sum
      ELSE
         soil_pH_root = 6.5  ! default neutral pH if no roots
      ENDIF
   
   ! Based on: log(Aarea) = 0.49 + 0.0809*pH
   ! f(pH) = 10^(0.0809 * (pH - pH_ref))
      pH_ref = 6.5
      pH_factor = 10.0**(0.0809 * (soil_pH_root - pH_ref))
      vm = vm * pH_factor
   ELSE
      pH_factor = 1.0
   ENDIF
            ...
            ! write(*,*) 'Vcmax : soil_pH_root =', soil_pH_root, ' pH_factor =', pH_factor
#endif
...
!     gbh2o  = gbh2o * cint(3)
   ...
   SUBROUTINE update_photosyn(nl_soil, tlef, po2m, pco2m, pco2a, par, psrf, rstfac, rb, gsh2o,&
                             effcon, vmax25, c3c4, gradm, trop, slti, hlti, shti, hhti, trda, trdm, cint, pH_vr, rootr,&
                             assim, respc)
