...
MODULE MOD_Vars_1DAccFluxes
   ...
   real(r8), allocatable :: a_er                 (:)
   real(r8), allocatable :: a_soil_er            (:)
...
!END plant hydraulic parameters
   ...
   real(r8), allocatable :: a_smin_no3_vr (:,:)
   real(r8), allocatable :: a_smin_nh4_vr (:,:)
   real(r8), allocatable :: a_pH_vr       (:,:)
   real(r8), allocatable :: a_pH_init_vr  (:,:)
   ! real(r8), allocatable :: a_calcite_mass(:,:)
   real(r8), allocatable :: a_calcite_content(:,:)
   real(r8), allocatable :: a_h_from_N    (:,:)
   real(r8), allocatable :: a_rate_cal    (:,:)
   real(r8), allocatable :: a_rate_CO2    (:,:)
   real(r8), allocatable :: a_rate_Bcex   (:,:)
   real(r8), allocatable :: a_rate_Hex    (:,:)
   real(r8), allocatable :: a_pCO2_soil   (:,:)
   real(r8), allocatable :: a_E_Bc        (:,:)
   real(r8), allocatable :: a_CEC         (:,:)
   real(r8), allocatable :: a_h_conc_vr   (:,:)
   real(r8), allocatable :: a_ca_conc_vr  (:,:)
   real(r8), allocatable :: a_DIC_conc_vr (:,:)
   real(r8), allocatable :: a_ANC_conc_vr (:,:)
   real(r8), allocatable :: a_Bc_conc_vr  (:,:)
   real(r8), allocatable :: a_Bct_conc_vr (:,:)
   real(r8), allocatable :: a_al3_conc_vr (:,:)
   ...
   IMPLICIT NONE
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            ...
            allocate (a_er                 (numpatch))
            allocate (a_soil_er            (numpatch))
...
!End Plant Hydraulic parameters
            ...
            allocate (a_smin_no3_vr (1:nl_soil,       numpatch))
            allocate (a_smin_nh4_vr (1:nl_soil,       numpatch))
            allocate (a_pH_vr       (1:nl_soil,       numpatch))
            allocate (a_pH_init_vr  (1:nl_soil,       numpatch))
            ! allocate (a_calcite_mass(1:nl_soil,       numpatch))
            allocate (a_calcite_content(1:nl_soil,     numpatch))
            allocate (a_h_from_N    (1:nl_soil,       numpatch))
            allocate (a_rate_cal    (1:nl_soil,       numpatch))  
            allocate (a_rate_CO2    (1:nl_soil,       numpatch))  
            allocate (a_rate_Bcex   (1:nl_soil,       numpatch))  
            allocate (a_rate_Hex    (1:nl_soil,       numpatch))  
            allocate (a_pCO2_soil   (1:nl_soil,       numpatch)) 
            allocate (a_E_Bc        (1:nl_soil,       numpatch))
            allocate (a_CEC         (1:nl_soil,       numpatch))
            allocate (a_h_conc_vr   (1:nl_soil,       numpatch))
            allocate (a_ca_conc_vr  (1:nl_soil,       numpatch))
            allocate (a_DIC_conc_vr (1:nl_soil,       numpatch))  
            allocate (a_ANC_conc_vr (1:nl_soil,       numpatch)) 
            allocate (a_Bc_conc_vr  (1:nl_soil,       numpatch))
            allocate (a_Bct_conc_vr (1:nl_soil,       numpatch))
            allocate (a_al3_conc_vr (1:nl_soil,       numpatch))
...
!END plant hydraulic parameters
            ...
            deallocate (a_smin_no3_vr )
            deallocate (a_smin_nh4_vr )
            deallocate (a_pH_vr       )
            deallocate (a_pH_init_vr  )
            ! deallocate (a_calcite_mass)
            deallocate (a_calcite_content)
            deallocate (a_h_from_N    )
            deallocate (a_rate_cal    )
            deallocate (a_rate_CO2    )
            deallocate (a_rate_Bcex   )
            deallocate (a_rate_Hex    )
            deallocate (a_pCO2_soil   )
            deallocate (a_E_Bc        )
            deallocate (a_CEC         )
            deallocate (a_h_conc_vr   )
            deallocate (a_ca_conc_vr  )
            deallocate (a_DIC_conc_vr )
            deallocate (a_ANC_conc_vr )
            deallocate (a_Bc_conc_vr  )
            deallocate (a_Bct_conc_vr )
            deallocate (a_al3_conc_vr  )
...
!END plant hydraulic parameters
            ...
            a_pH_vr        (:,:) = spval
            a_pH_init_vr   (:,:) = spval
            ! a_calcite_mass (:,:) = spval
            a_calcite_content(:,:) = spval
            a_h_from_N     (:,:) = spval
            a_rate_cal     (:,:) = spval
            a_rate_CO2     (:,:) = spval
            a_rate_Bcex    (:,:) = spval
   ...
   real(r8) um,thvstar,beta,zii,wc,wc2
      ...
      IF (p_is_worker) THEN
         IF (numpatch > 0) THEN
            ...
            CALL acc2d (smin_no3_vr  , a_smin_no3_vr )
            CALL acc2d (smin_nh4_vr  , a_smin_nh4_vr )
            CALL acc2d (pH_vr        , a_pH_vr       )
            CALL acc2d (pH_init_vr   , a_pH_init_vr  )
            ! CALL acc2d (calcite_mass , a_calcite_mass)
            CALL acc2d (calcite_content  , a_calcite_content )
            CALL acc2d (h_from_N     , a_h_from_N    )
            CALL acc2d (rate_cal     , a_rate_cal    )
            CALL acc2d (rate_CO2     , a_rate_CO2    )
            CALL acc2d (rate_Bcex    , a_rate_Bcex   )
            CALL acc2d (rate_Hex     , a_rate_Hex    )
            CALL acc2d (pCO2_soil    , a_pCO2_soil   )
            CALL acc2d (E_Bc         , a_E_Bc        )
            CALL acc2d (CEC          , a_CEC         )
            CALL acc2d (h_conc_vr    , a_h_conc_vr   )
            CALL acc2d (ca_conc_vr   , a_ca_conc_vr  )
            CALL acc2d (DIC_conc_vr  , a_DIC_conc_vr )
            CALL acc2d (ANC_conc_vr  , a_ANC_conc_vr )
            CALL acc2d (Bc_conc_vr   , a_Bc_conc_vr  )
            CALL acc2d (Bct_conc_vr  , a_Bct_conc_vr )
            CALL acc2d (al3_conc_vr  , a_al3_conc_vr )
