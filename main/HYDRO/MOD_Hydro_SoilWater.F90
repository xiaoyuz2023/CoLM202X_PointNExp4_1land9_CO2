...
!-------------------------------------------------------------------------
   ...
   USE MOD_Namelist, only: DEF_USE_PLANTHYDRAULICS, DEF_USE_SOILACIDIFICATION
...
CONTAINS
   ...
   SUBROUTINE soil_water_vertical_movement (                      &
         ...
         tolerance,  wblc, &
         t_soisno, wice_soisno, forc_pco2m, pH_vr, pH_init_vr, E_Bc, &
         h_conc_vr, oh_conc_vr, ca_conc_vr, Bc_conc_vr,&
         Bct_conc_vr, al3_conc_vr, al2_conc_vr, al_conc_vr, &
         DIC_conc_vr, h2co3_conc_vr, hco3_conc_vr, co3_conc_vr, &
         h_conc_m1, oh_conc_m1, ca_conc_m1, Bc_conc_m1, DIC_conc_m1)
   ...
   real(r8), intent(in) :: t_soisno(1:nlev)
   real(r8), intent(in) :: wice_soisno(1:nlev)
   real(r8), intent(in) :: forc_pco2m
   real(r8), intent(inout) :: pH_vr(1:nlev)
   real(r8), intent(inout) :: pH_init_vr(1:nlev)
   real(r8), intent(inout) :: E_Bc(1:nlev)
   real(r8), intent(inout) :: h_conc_vr(1:nlev)
   real(r8), intent(inout) :: oh_conc_vr(1:nlev)
   real(r8), intent(inout) :: ca_conc_vr(1:nlev)
   real(r8), intent(inout) :: Bc_conc_vr(1:nlev)
   real(r8), intent(inout) :: Bct_conc_vr(1:nlev)
   real(r8), intent(inout) :: al3_conc_vr(1:nlev)
   real(r8), intent(inout) :: al2_conc_vr(1:nlev)
   real(r8), intent(inout) :: al_conc_vr(1:nlev)
   real(r8), intent(inout) :: DIC_conc_vr(1:nlev)
   real(r8), intent(inout) :: h2co3_conc_vr(1:nlev)
   real(r8), intent(inout) :: hco3_conc_vr(1:nlev)
   real(r8), intent(inout) :: co3_conc_vr(1:nlev)
   real(r8), intent(inout) :: h_conc_m1(1:nlev)
   real(r8), intent(inout) :: oh_conc_m1(1:nlev)
   real(r8), intent(inout) :: ca_conc_m1(1:nlev)
   real(r8), intent(inout) :: Bc_conc_m1(1:nlev)
   real(r8), intent(inout) :: DIC_conc_m1(1:nlev)
   ...
   real(r8) :: alpha_L = 10.0_r8 ! mm
   real(r8) :: K_HBc = 1000_r8
   real(r8) :: zero_etroot (1:nlev), Base
   real(r8) :: h_flux_disp(0:nlev), oh_flux_disp(0:nlev), ca_flux_disp(0:nlev), Bc_flux_disp(0:nlev), DIC_flux_disp(0:nlev), cl_flux_disp(0:nlev)
   real(r8) :: h_flux_adv(1:nlev), oh_flux_adv(1:nlev), ca_flux_adv(1:nlev), Bc_flux_adv(1:nlev), DIC_flux_adv(1:nlev), cl_flux_adv(1:nlev)
   real(r8) :: wliq_m1(1:nlev), water_in, water_out, h_input, oh_input
   real(r8) :: ss_vliq_old(1:nlev), ss_vliq_new(1:nlev)
   real(r8) :: K_H(1:nlev), K1(1:nlev), K2(1:nlev), Kw(1:nlev), log_KH, log_K1, log_K2, log_Kw
   real(r8) :: log_Ksp_calcite, Ksp_calcite(1:nlev), log_Kal_3, Kal_3(1:nlev), log_Kal_1, Kal_1(1:nlev), log_Kal_2, Kal_2(1:nlev)
   real(r8) :: wliq_old(1:nlev), wliq_final(1:nlev), tot_water_before(1:nlev), tot_water_final(1:nlev)
   real(r8) :: h_conc_old(1:nlev), oh_conc_old(1:nlev), ca_conc_old(1:nlev), Bc_conc_old(1:nlev), DIC_conc_old(1:nlev)
   real(r8) :: h_mass_ice_before(1:nlev), oh_mass_ice_before(1:nlev), ca_mass_ice_before(1:nlev), Bc_mass_ice_before(1:nlev), DIC_mass_ice_before(1:nlev)
   real(r8) :: h_mass_wliq_after(1:nlev), oh_mass_wliq_after(1:nlev), ca_mass_wliq_after(1:nlev), Bc_mass_wliq_after(1:nlev), DIC_mass_wliq_after(1:nlev)  
   real(r8) :: h_mass_before(1:nlev), oh_mass_before(1:nlev), ca_mass_before(1:nlev), Bc_mass_before(1:nlev), DIC_mass_before(1:nlev), h_mass_after(1:nlev), oh_mass_after(1:nlev), ca_mass_after(1:nlev), Bc_mass_after(1:nlev), DIC_mass_after(1:nlev)
   real(r8) :: h_mass_error, oh_mass_error, ca_mass_error, Bc_mass_error, DIC_mass_error, h_mass_rel_error, oh_mass_rel_error, ca_mass_rel_error, Bc_mass_rel_error, DIC_mass_rel_error
   real(r8) :: h_mass_tot_before, oh_mass_tot_before, ca_mass_tot_before, Bc_mass_tot_before, DIC_mass_tot_before, h_mass_tot_after, oh_mass_tot_after, ca_mass_tot_after, Bc_mass_tot_after, DIC_mass_tot_after
   real(r8) :: h_flux_net, oh_flux_net, ca_flux_net, Bc_flux_net, DIC_flux_net, ss_vliq_m1_check, ice_fraction, ss_vliq_min = 5e-2_r8
   logical :: skip_flag(1:nlev), first_call = .true.
   logical :: h_corrected(1:nlev), oh_corrected(1:nlev), ca_corrected(1:nlev), Bc_corrected(1:nlev), DIC_corrected(1:nlev)


      sp_dz(1:nlev) = sp_zi(1:nlev) - sp_zi(0:nlev-1)
      
      IF(DEF_USE_SOILACIDIFICATION)THEN
         DO ilev = 1, nlev
            ss_vliq_old(ilev) = ss_vliq(ilev)
            zero_etroot(ilev) = 0.0_r8
            h_conc_m1(ilev) = h_conc_vr(ilev)
            oh_conc_m1(ilev) = oh_conc_vr(ilev)
            ca_conc_m1(ilev) = ca_conc_vr(ilev)
            DIC_conc_m1(ilev) = DIC_conc_vr(ilev)
            Bc_conc_m1(ilev) = Bc_conc_vr(ilev)
         ENDDO

         IF (first_call) THEN
            DO ilev = 1, nlev
               pH_vr(ilev) = 6.5_r8
               pH_init_vr(ilev) = 6.5_r8
               E_Bc(ilev) = 80_r8
               h_conc_vr(ilev) = 10.0_r8**(-pH_vr(ilev))
               oh_conc_vr(ilev) = 1e-14_r8 / h_conc_vr(ilev)
               DIC_conc_vr(ilev) = 1e-3_r8
               IF (pH_init_vr(ilev) > 6.2_r8) THEN
                  ca_conc_vr(ilev) = 1e-4_r8
                  Bc_conc_vr(ilev) = MAX(ca_conc_vr(ilev) * 0.3_r8, 1.0e-20_r8)
                  Bct_conc_vr(ilev) = ca_conc_vr(ilev) + Bc_conc_vr(ilev)
               ELSE
                  IF (E_Bc(ilev) < 99.9_r8) THEN
                     Base = E_Bc(ilev)/100.0_r8
                     Bct_conc_vr(ilev) = ((Base / (1.0_r8 - Base)) * K_HBc * h_conc_vr(ilev))**2.0_r8
                     Bct_conc_vr(ilev) = MAX(Bct_conc_vr(ilev), 1.0e-20_r8)
                  ELSE
                     Bct_conc_vr(ilev) = 1.0e-2_r8
                  ENDIF
                  ca_conc_vr(ilev) = MAX(Bct_conc_vr(ilev) * 0.4_r8, 1.0e-20_r8)
                  Bc_conc_vr(ilev) = MAX(Bct_conc_vr(ilev) * 0.6_r8, 1.0e-20_r8)
               ENDIF
               Bct_conc_vr(ilev) = MAX(ca_conc_vr(ilev) + Bc_conc_vr(ilev), 1.0e-20_r8) 
               h_conc_m1(ilev) = h_conc_vr(ilev)
               oh_conc_m1(ilev) = oh_conc_vr(ilev)
               ca_conc_m1(ilev) = ca_conc_vr(ilev)
               Bc_conc_m1(ilev) = Bc_conc_vr(ilev)
               ...
               write(*,*) 'INIT layer =', ilev, 'pH =', pH_vr(ilev), 'pH_init =', pH_init_vr(ilev),'DIC =', DIC_conc_vr(ilev), &
                           'H+ =', h_conc_vr(ilev), 'OH- =', oh_conc_vr(ilev), &
                           'Bc =', Bc_conc_vr(ilev), 'Bct =',Bct_conc_vr(ilev), 'Ca =', ca_conc_vr(ilev),'E_Bc =', E_Bc(ilev)
            ENDDO
            first_call = .false.
         ENDIF
      ENDIF
      ...
      IF(DEF_USE_SOILACIDIFICATION) THEN
         h_mass_tot_before = 0.0_r8
         oh_mass_tot_before = 0.0_r8
         ca_mass_tot_before = 0.0_r8
         Bc_mass_tot_before = 0.0_r8
         DIC_mass_tot_before = 0.0_r8

         ! 1. 保存原始浓度和计算初始溶质质量
         DO ilev = 1, nlev
            wliq_old(ilev) = ss_vliq_old(ilev) * sp_dz(ilev)
            tot_water_before(ilev) = wliq_old(ilev) + wice_soisno(ilev)
            
            ! 保存原始浓度（在分离冰中溶质之前）
            h_conc_old(ilev) = h_conc_vr(ilev)
            oh_conc_old(ilev) = oh_conc_vr(ilev)
            ca_conc_old(ilev) = ca_conc_vr(ilev)
            Bc_conc_old(ilev) = Bc_conc_vr(ilev)
            DIC_conc_old(ilev) = DIC_conc_vr(ilev)
            
            ! 计算所有层的初始总质量
            IF (tot_water_before(ilev) > 1.e-10_r8) THEN
               h_mass_tot_before = h_mass_tot_before + h_conc_vr(ilev) * tot_water_before(ilev)
               oh_mass_tot_before = oh_mass_tot_before + oh_conc_vr(ilev) * tot_water_before(ilev)
               ca_mass_tot_before = ca_mass_tot_before + ca_conc_vr(ilev) * tot_water_before(ilev)
               Bc_mass_tot_before = Bc_mass_tot_before + Bc_conc_vr(ilev) * tot_water_before(ilev)
               DIC_mass_tot_before = DIC_mass_tot_before + DIC_conc_vr(ilev) * tot_water_before(ilev)
            ENDIF
         ENDDO

         ! 2. 计算水流通量（用于运移计算）
         DO ilev = 1, nlev
            ss_vliq_new(ilev) = ss_vliq(ilev)
            water_in  = 0.0_r8
            water_out = 0.0_r8
            IF (ilev == 1) THEN
               water_in  = water_in  + MAX(ss_q(0), 0.0_r8) * dt
               water_out = water_out + MAX(-ss_q(0), 0.0_r8) * dt
            ENDIF

            IF (ilev > 1) THEN
               water_in  = water_in  + MAX(ss_q(ilev-1), 0.0_r8) * dt
               water_out = water_out + MAX(-ss_q(ilev-1), 0.0_r8) * dt
            ENDIF

            IF (ilev < nlev) THEN
               water_out = water_out + MAX(ss_q(ilev), 0.0_r8) * dt
               water_in  = water_in  + MAX(-ss_q(ilev), 0.0_r8) * dt
            ENDIF

            IF (ilev == nlev) THEN
               water_out = water_out + MAX(ss_q(nlev), 0.0_r8) * dt
               water_in  = water_in  + MAX(-ss_q(nlev), 0.0_r8) * dt
            ENDIF
            water_out = water_out + etroot(ilev) * dt
            wliq_m1(ilev) = ss_vliq_old(ilev) * sp_dz(ilev) + (water_in - water_out) ! 用于运移计算中的预测值，表示水流运移后的液态水体积(kg/m2)

            ss_vliq_m1_check = wliq_m1(ilev) / sp_dz(ilev)
            skip_flag(ilev) = (ss_vliq_m1_check <= ss_vliq_min .OR. ss_vliq_new(ilev) <= ss_vliq_min)
         ENDDO

         ! 3. 分离冰中的溶质（冰中溶质不参与运移）
         DO ilev = 1, nlev
            IF (skip_flag(ilev)) CYCLE
            
            ! 参与运移：分离冰中溶质
            IF (tot_water_before(ilev) > 1.e-10_r8 .AND. wice_soisno(ilev) > 1.e-10_r8) THEN
               ! 计算冰中固定的溶质比例
               ice_fraction = wice_soisno(ilev) / tot_water_before(ilev) ! 假设是冰中溶质质量与总水中的浓度成比例
               
               ! 保存各离子在冰中的质量
               h_mass_ice_before(ilev) = h_conc_vr(ilev) * tot_water_before(ilev) * ice_fraction
               oh_mass_ice_before(ilev) = oh_conc_vr(ilev) * tot_water_before(ilev) * ice_fraction
               ca_mass_ice_before(ilev) = ca_conc_vr(ilev) * tot_water_before(ilev) * ice_fraction
               Bc_mass_ice_before(ilev) = Bc_conc_vr(ilev) * tot_water_before(ilev) * ice_fraction
               DIC_mass_ice_before(ilev) = DIC_conc_vr(ilev) * tot_water_before(ilev) * ice_fraction
               
               ! 假设只有液态水中的溶质才能通过平流和弥散进行运移
               IF (wliq_old(ilev) > 1.e-10_r8) THEN
                  h_conc_vr(ilev) = (h_conc_vr(ilev) * tot_water_before(ilev) - h_mass_ice_before(ilev)) / wliq_old(ilev)
                  oh_conc_vr(ilev) = (oh_conc_vr(ilev) * tot_water_before(ilev) - oh_mass_ice_before(ilev)) / wliq_old(ilev)
                  ca_conc_vr(ilev) = (ca_conc_vr(ilev) * tot_water_before(ilev) - ca_mass_ice_before(ilev)) / wliq_old(ilev)
                  Bc_conc_vr(ilev) = (Bc_conc_vr(ilev) * tot_water_before(ilev) - Bc_mass_ice_before(ilev)) / wliq_old(ilev)
                  DIC_conc_vr(ilev) = (DIC_conc_vr(ilev) * tot_water_before(ilev) - DIC_mass_ice_before(ilev)) / wliq_old(ilev)
               ENDIF
            ELSE
               ! 没有冰或总水量太少
               h_mass_ice_before(ilev) = 0.0_r8
               oh_mass_ice_before(ilev) = 0.0_r8
               ca_mass_ice_before(ilev) = 0.0_r8
               Bc_mass_ice_before(ilev) = 0.0_r8
               DIC_mass_ice_before(ilev) = 0.0_r8
            ENDIF
         ENDDO
   
         ! 4. 溶质运移
         h_input = 10.0_r8**(-5.6_r8)
         oh_input = 10.0_r8**(-8.4_r8)
         ! h_input = 10.0_r8**(-7.5_r8)
         ! oh_input = 10.0_r8**(-6.5_r8)
         ...
         ! write(*,*) '------ H+ transport ------'
         CALL calculate_ion_transport( &
            nlev, dt, sp_dz, ss_q, zero_etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
            h_input, h_input, alpha_L, 9.3e-3_r8, 1e-8_r8, 1e-14_r8, &
            h_conc_vr, h_flux_adv, h_flux_disp, h_flux_net, h_corrected )

         ! write(*,*) '------ OH- transport ------'
         CALL calculate_ion_transport( &
            nlev, dt, sp_dz, ss_q, zero_etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
            oh_input, oh_input, alpha_L, 5.3e-3_r8, 1e-8_r8, 1e-14_r8, &
            oh_conc_vr, oh_flux_adv, oh_flux_disp, oh_flux_net, oh_corrected )
            
         ! write(*,*) '------ Ca2+ transport ------'
         CALL calculate_ion_transport( &
            nlev, dt, sp_dz, ss_q, etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
            7e-6_r8, 7e-6_r8, alpha_L, 7.9e-4_r8, 1e-3_r8, 1e-10_r8, & 
            ca_conc_vr, ca_flux_adv, ca_flux_disp, ca_flux_net, ca_corrected ) 

         ! write(*,*) '------  Bc2+ (K+, Mg2+) transport ------'
         CALL calculate_ion_transport( &
            nlev, dt, sp_dz, ss_q, etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
            3e-6_r8, 3e-6_r8, alpha_L, 7.9e-4_r8, 1e-3_r8, 1e-10_r8, & 
            Bc_conc_vr, Bc_flux_adv, Bc_flux_disp, Bc_flux_net, Bc_corrected )

         ! write(*,*) '------ DIC transport ------'
         CALL calculate_ion_transport( &
            nlev, dt, sp_dz, ss_q, zero_etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
            0.0_r8, 0.0_r8, alpha_L, 1.2e-3_r8, 1e-3_r8, 1e-10_r8, & 
            DIC_conc_vr, DIC_flux_adv, DIC_flux_disp, DIC_flux_net, DIC_corrected )
      ...
      IF(DEF_USE_SOILACIDIFICATION) THEN
         DO ilev = 1, nlev
            ! 最终的液态水量和总水量
            wliq_final(ilev) = ss_vliq(ilev) * sp_dz(ilev)
            tot_water_final(ilev) = wliq_final(ilev) + wice_soisno(ilev)
            IF (skip_flag(ilev)) THEN  ! 不参与运移：保持原始浓度, 不考虑水量变化
               h_conc_vr(ilev) = h_conc_old(ilev)
               oh_conc_vr(ilev) = oh_conc_old(ilev)
               ca_conc_vr(ilev) = ca_conc_old(ilev)
               Bc_conc_vr(ilev) = Bc_conc_old(ilev)
               DIC_conc_vr(ilev) = DIC_conc_old(ilev)
            ELSE
               ! ========== 该层参与了运移 ==========
               IF (tot_water_final(ilev) > 1.e-10_r8) THEN
                  ! 运移后液态水中的溶质质量（h_conc_vr 已经是运移后的液态水浓度）
                  h_mass_wliq_after(ilev) = h_conc_vr(ilev) * wliq_final(ilev)
                  oh_mass_wliq_after(ilev) = oh_conc_vr(ilev) * wliq_final(ilev)
                  ca_mass_wliq_after(ilev) = ca_conc_vr(ilev) * wliq_final(ilev)
                  Bc_mass_wliq_after(ilev) = Bc_conc_vr(ilev) * wliq_final(ilev)
                  DIC_mass_wliq_after(ilev) = DIC_conc_vr(ilev) * wliq_final(ilev)
                  
                  ! 冰中固定的溶质质量（保持不变）
                  ! 总溶质质量 = 液态水中的质量 + 冰中固定的质量
                  h_mass_after(ilev) = h_mass_wliq_after(ilev) + h_mass_ice_before(ilev)
                  oh_mass_after(ilev) = oh_mass_wliq_after(ilev) + oh_mass_ice_before(ilev)
                  ca_mass_after(ilev) = ca_mass_wliq_after(ilev) + ca_mass_ice_before(ilev)
                  Bc_mass_after(ilev) = Bc_mass_wliq_after(ilev) + Bc_mass_ice_before(ilev)
                  DIC_mass_after(ilev) = DIC_mass_wliq_after(ilev) + DIC_mass_ice_before(ilev)
                  
                  ! === 关键修改：只对未被渐进式限制器修正的层重新计算浓度 ===
                  ! 更新浓度：溶质均匀分布在最终总水量中
                  IF (.not. h_corrected(ilev)) THEN
                     h_conc_vr(ilev) = h_mass_after(ilev) / tot_water_final(ilev)
                  ENDIF
                  
                  IF (.not. oh_corrected(ilev)) THEN
                     oh_conc_vr(ilev) = oh_mass_after(ilev) / tot_water_final(ilev)
                  ENDIF
                  
                  IF (.not. ca_corrected(ilev)) THEN
                     ca_conc_vr(ilev) = ca_mass_after(ilev) / tot_water_final(ilev)
                  ENDIF
                  
                  IF (.not. Bc_corrected(ilev)) THEN
                     Bc_conc_vr(ilev) = Bc_mass_after(ilev) / tot_water_final(ilev)
                  ENDIF
                  
                  IF (.not. DIC_corrected(ilev)) THEN
                     DIC_conc_vr(ilev) = DIC_mass_after(ilev) / tot_water_final(ilev)
                  ENDIF
               ENDIF
            ENDIF
            IF(h_corrected(ilev).or. oh_corrected(ilev).or. ca_corrected(ilev).or. Bc_corrected(ilev).or. DIC_corrected(ilev)) THEN
               write(*,*) 'Layer', ilev,'[H], [OH], [Ca2+], [Bc2+], [DIC] =', h_corrected(ilev), oh_corrected(ilev), ca_corrected(ilev), Bc_corrected(ilev), DIC_corrected(ilev)
            ENDIF
         ENDDO
         
         ! 6. 验证质量守恒
         h_mass_tot_after = 0.0_r8
         oh_mass_tot_after = 0.0_r8
         ca_mass_tot_after = 0.0_r8
         Bc_mass_tot_after = 0.0_r8
         DIC_mass_tot_after = 0.0_r8
         h_mass_rel_error = 0.0_r8
         oh_mass_rel_error = 0.0_r8
         ca_mass_rel_error = 0.0_r8
         Bc_mass_rel_error = 0.0_r8
         DIC_mass_tot_after = 0.0_r8
         
         DO ilev = 1, nlev
            IF (tot_water_final(ilev) > 1.e-10_r8) THEN
               h_mass_tot_after = h_mass_tot_after + h_conc_vr(ilev) * tot_water_final(ilev)
               oh_mass_tot_after = oh_mass_tot_after + oh_conc_vr(ilev) * tot_water_final(ilev)
               ca_mass_tot_after = ca_mass_tot_after + ca_conc_vr(ilev) * tot_water_final(ilev)
               Bc_mass_tot_after = Bc_mass_tot_after + Bc_conc_vr(ilev) * tot_water_final(ilev)
               DIC_mass_tot_after = DIC_mass_tot_after + DIC_conc_vr(ilev) * tot_water_final(ilev)
            ENDIF
         ENDDO

         ! 修正后的全局质量守恒绝对误差
         h_mass_error = abs((h_mass_tot_before + h_flux_net*dt) - h_mass_tot_after)
         oh_mass_error = abs((oh_mass_tot_before + oh_flux_net*dt) - oh_mass_tot_after)
         ca_mass_error = abs((ca_mass_tot_before + ca_flux_net*dt) - ca_mass_tot_after)
         Bc_mass_error = abs((Bc_mass_tot_before + Bc_flux_net*dt) - Bc_mass_tot_after)
         DIC_mass_error = abs((DIC_mass_tot_before + DIC_flux_net*dt) - DIC_mass_tot_after)

         ! 计算相对误差
         IF (h_mass_tot_before > 1.e-20_r8) THEN
            h_mass_rel_error = h_mass_error / h_mass_tot_before
         ENDIF
         
         IF (oh_mass_tot_before > 1.e-20_r8) THEN
            oh_mass_rel_error = oh_mass_error / oh_mass_tot_before
         ENDIF
         
         IF (ca_mass_tot_before > 1.e-20_r8) THEN
            ca_mass_rel_error = ca_mass_error / ca_mass_tot_before
         ENDIF
         
         IF (Bc_mass_tot_before > 1.e-20_r8) THEN
            Bc_mass_rel_error = Bc_mass_error / Bc_mass_tot_before
         ENDIF

         IF (DIC_mass_tot_before > 1.e-20_r8) THEN
            DIC_mass_rel_error = DIC_mass_error / DIC_mass_tot_before
         ENDIF
         
         ! 如果质量守恒误差过大，给出警告
         IF (h_mass_rel_error > 0.01_r8 .OR. oh_mass_rel_error > 0.01_r8 .OR. &
             ca_mass_rel_error > 0.01_r8 .or. Bc_mass_rel_error > 0.01_r8 .or. DIC_mass_rel_error >0.01_r8) THEN
            write(*,*) 'Warnning: Mass balance error > 1% detected!'
         ENDIF

         DO ilev = 1, nlev
            IF (pH_init_vr(ilev) /= spval) THEN
               pH_vr(ilev) = -log10(h_conc_vr(ilev))
            ENDIF
         ENDDO
      ENDIF
   ...
   SUBROUTINE calculate_ion_transport( &
         nlev, dt, sp_dz, ss_q, etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
         rainfall_ion_conc, groundwater_ion_conc, alpha_L, D_star, tol_ion, ion_min, &
         ion_conc_vr, ion_flux_adv, ion_flux_disp, ion_flux_net, ion_corrected)

      IMPLICIT NONE
      integer,  intent(in)       :: nlev
      real(r8), intent(in)       :: dt
      real(r8), intent(in)       :: sp_dz(1:nlev)
      real(r8), intent(in)       :: ss_q(0:nlev) ! mm/s Darcy水流速度
      real(r8), intent(in)       :: etroot(1:nlev)
      real(r8), intent(in)       :: ss_vliq_old(1:nlev), ss_vliq_new(1:nlev)
      real(r8), intent(in)       :: wliq_m1(1:nlev)
      real(r8), intent(in)       :: rainfall_ion_conc, groundwater_ion_conc  ! mol/l
      real(r8), intent(in)       :: alpha_L, D_star
      real(r8), intent(in)       :: tol_ion
      real(r8), intent(in)       :: ion_min
      real(r8), intent(inout)    :: ion_conc_vr(1:nlev)
      real(r8), intent(out)      :: ion_flux_adv(1:nlev)  ! mol/l/s
      real(r8), intent(out)      :: ion_flux_disp(0:nlev) ! mol/m2/s
      real(r8), intent(out)      :: ion_flux_net ! mol/m2/s
      logical,  intent(out)      :: ion_corrected(1:nlev) ! 标记哪些层被修正过

      CALL calculate_advection_explicit(nlev, dt, sp_dz, ss_q, etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
                                       rainfall_ion_conc, groundwater_ion_conc, tol_ion, ion_min, &
                                       ion_conc_vr, ion_flux_adv, ion_flux_net, ion_corrected)

      ! CALL calculate_dispersion_implicit(nlev, dt, sp_dz, ss_q, ss_vliq_new, &
      !                                  rainfall_ion_conc, groundwater_ion_conc, alpha_L, D_star, tol_ion, &
      !                                  ion_conc_vr, ion_flux_disp)


   END SUBROUTINE calculate_ion_transport

 
   ! --- 显式对流-迎风格式 ---
   SUBROUTINE calculate_advection_explicit(nlev, dt, sp_dz, q, etroot, ss_vliq_old, ss_vliq_new, wliq_m1, &
                                       rainfall_ion_conc, groundwater_ion_conc, tol_ion, ion_min, &
                                       ion_conc_vr, ion_flux_adv, ion_flux_net, ion_corrected)
      IMPLICIT NONE
      integer,  intent(in)       :: nlev
      real(r8), intent(in)       :: dt
      real(r8), intent(in)       :: sp_dz(1:nlev)
      real(r8), intent(in)       :: q(0:nlev) ! mm/s Darcy水流速度
      real(r8), intent(in)       :: etroot(1:nlev)
      real(r8), intent(in)       :: ss_vliq_old(1:nlev), ss_vliq_new(1:nlev), wliq_m1(1:nlev)
      real(r8), intent(in)       :: rainfall_ion_conc, groundwater_ion_conc  ! mol/l
      real(r8), intent(in)       :: tol_ion
      real(r8), intent(in)       :: ion_min
      real(r8), intent(inout)    :: ion_conc_vr(1:nlev)
      real(r8), intent(out)      :: ion_flux_adv(1:nlev)  ! mol/l/s
      real(r8), intent(out)      :: ion_flux_net ! mol/m2/s
      logical,  intent(out)      :: ion_corrected(1:nlev) ! 标记哪些层被修正

      integer :: ilev
      real(r8) :: ion_blc, wliq_min = 1.0e-1_r8, ss_vliq_min = 5e-2_r8, max_decrease_factor = 0.4_r8
      real(r8) :: weight_up, weight_down, weighted_conc
      real(r8) :: ion_flux_transp(1:nlev), ion_flux_in(1:nlev), ion_flux_out(1:nlev), ion_flux_source, ion_flux_sink
      real(r8) :: ion_mass_old(1:nlev), ion_mass_new(1:nlev) ! mol/m2
      real(r8) :: wliq_new(1:nlev), ss_vliq_m1(1:nlev), ion_before, mass_before_correction

      ion_flux_source = 0.0_r8
      ion_flux_sink = 0.0_r8
      ion_corrected(:) = .false.  ! 初始化：所有层都未修正

      DO ilev = 1, nlev
         ion_before = ion_conc_vr(ilev)
         wliq_new(ilev) = ss_vliq_new(ilev)*sp_dz(ilev)
         ss_vliq_m1(ilev) = wliq_m1(ilev)/sp_dz(ilev)

         ion_flux_in(ilev) = 0.0_r8
         ion_flux_out(ilev) = 0.0_r8
         ion_flux_transp(ilev) = 0.0_r8
         ion_flux_adv(ilev) = 0.0_r8

         weight_up = 1.0_r8
         weight_down = 1.0_r8 - weight_up

         ! 包括降雨、入渗、蒸发和地下水交换, q > 0 表示从上层流入下层
         IF (ilev == 1) THEN
            IF (q(0) > 0) THEN
               ion_flux_in(ilev) = ion_flux_in(ilev) + q(ilev-1) * rainfall_ion_conc
               ion_flux_source = ion_flux_source + q(ilev-1) * rainfall_ion_conc
            ELSE
               ion_flux_out(ilev) = ion_flux_out(ilev) - q(ilev-1) * ion_conc_vr(ilev)
               ion_flux_sink = ion_flux_sink - q(ilev-1) * ion_conc_vr(ilev)
            ENDIF
         ENDIF

         IF (ilev > 1) THEN
            IF (q(ilev-1) > 0) THEN
               weighted_conc = weight_up * ion_conc_vr(ilev-1) + weight_down * ion_conc_vr(ilev)
               ion_flux_in(ilev) = ion_flux_in(ilev) + q(ilev-1) * weighted_conc
            ELSE
               weighted_conc = weight_up * ion_conc_vr(ilev) + weight_down * ion_conc_vr(ilev-1)
               ion_flux_out(ilev) = ion_flux_out(ilev) - q(ilev-1) * weighted_conc
            ENDIF
         ENDIF
         ...
         IF (ilev < nlev) THEN
            IF (q(ilev) > 0) THEN
               weighted_conc = weight_up * ion_conc_vr(ilev) + weight_down * ion_conc_vr(ilev+1)
               ion_flux_out(ilev) = ion_flux_out(ilev) + q(ilev) * weighted_conc
            ELSE
               weighted_conc = weight_up * ion_conc_vr(ilev+1) + weight_down * ion_conc_vr(ilev)
               ion_flux_in(ilev) = ion_flux_in(ilev) - q(ilev) * weighted_conc
            ENDIF
         ENDIF

         IF (ilev == nlev) THEN
            IF (q(nlev) > 0) THEN
               ion_flux_out(ilev) = ion_flux_out(ilev) + q(ilev) * ion_conc_vr(ilev)
               ion_flux_sink = ion_flux_sink + q(ilev) * ion_conc_vr(ilev)
            ELSE
               ion_flux_in(ilev) = ion_flux_in(ilev) - q(ilev) * groundwater_ion_conc
               ion_flux_source = ion_flux_source + q(ilev) * groundwater_ion_conc
            ENDIF
         ENDIF

         ion_flux_transp(ilev) = etroot(ilev) * ion_conc_vr(ilev)
         ion_flux_out(ilev) = ion_flux_out(ilev) + ion_flux_transp(ilev)
         ion_flux_sink  = ion_flux_sink + ion_flux_transp(ilev)  
         ion_flux_net = ion_flux_source - ion_flux_sink
        
         ion_flux_adv(ilev) = ion_flux_in(ilev) - ion_flux_out(ilev) ! mol/m2/s  
         ion_mass_old(ilev) = ion_conc_vr(ilev) * ss_vliq_old(ilev) * sp_dz(ilev)  
         
         IF (ss_vliq_m1(ilev) <= ss_vliq_min .or. ss_vliq_new(ilev) <= ss_vliq_min) THEN
            ion_mass_new(ilev) = ion_conc_vr(ilev) * wliq_m1(ilev)
            ! ion_mass_new(ilev) = ion_conc_vr(ilev) * wliq_new(ilev)
         ELSE
            ion_mass_new(ilev) = ion_mass_old(ilev) + ion_flux_adv(ilev) * dt
         ENDIF    

         IF (ss_vliq_m1(ilev) > ss_vliq_min .and. ss_vliq_new(ilev) > ss_vliq_min) THEN
            ion_conc_vr(ilev) = ion_mass_new(ilev) / wliq_m1(ilev)
            ! ion_conc_vr(ilev) = ion_mass_new(ilev) / wliq_new(ilev)
            
            IF (ion_conc_vr(ilev) < ion_min) THEN
               mass_before_correction = ion_mass_new(ilev)
               
               IF (ion_before > ion_min) THEN
                  ion_conc_vr(ilev) = MAX(ion_before * max_decrease_factor, ion_min)
               ELSE
                  ion_conc_vr(ilev) = ion_min
               ENDIF
               ion_mass_new(ilev) = ion_conc_vr(ilev) * wliq_m1(ilev)
               ion_corrected(ilev) = .true.
               write(*,*) 'Warning: Extreme precipitation, calibration concentration Layer =', ilev, 'ion_min=', ion_min
            ENDIF
         ENDIF

      ENDDO
      
      ion_blc = sum(ion_mass_old(1:nlev)) - sum(ion_mass_new(1:nlev))  + ion_flux_net * dt
      IF (ABS(ion_blc) > tol_ion) THEN
         write(*,*) 'Warning: HYDRO_advection balance error: ', ion_blc
      ENDIF
   END SUBROUTINE calculate_advection_explicit

   ! --- 隐式弥散-中心差分 ---
   SUBROUTINE calculate_dispersion_implicit(nlev, dt, sp_dz, q, ss_vliq_new, &
                                          rainfall_ion_conc, groundwater_ion_conc, alpha_L, D_star, tol_ion, &
                                          ion_conc_vr, ion_flux_disp)
      IMPLICIT NONE
      ...
      real(r8), intent(in)    :: dt
      real(r8), intent(in)    :: sp_dz(1:nlev)
      real(r8), intent(in)    :: q(0:nlev)
      real(r8), intent(in)    :: ss_vliq_new(1:nlev)
      real(r8), intent(in)    :: rainfall_ion_conc, groundwater_ion_conc ! mol/l
      real(r8), intent(in)    :: alpha_L, D_star
      real(r8), intent(in)    :: tol_ion
      real(r8), intent(inout) :: ion_conc_vr(1:nlev)
      ...
      integer  :: ilev
      real(r8) :: ion_mass_initial, ion_mass_final, ion_flux_net, ion_conc_old(1:nlev)
      real(r8) :: ion_blc, ss_vliq_min = 5e-2_r8
      real(r8) :: a(1:nlev), b(1:nlev), c(1:nlev), d(1:nlev)
      real(r8) :: D_intf(0:nlev), theta_intf, dz_intf
      logical  :: is_active_layer(1:nlev)
      ! 界面弥散系数, 界面体积含水量, 界面上的Darcy速度, 界面间的距离

      ion_mass_initial = 0.0_r8
      ion_mass_final = 0.0_r8

      DO ilev = 1, nlev
         is_active_layer(ilev) = (ss_vliq_new(ilev) >= ss_vliq_min)
         ion_conc_old(ilev) = ion_conc_vr(ilev)

         IF (is_active_layer(ilev)) THEN
            ion_mass_initial = ion_mass_initial + ion_conc_old(ilev)*ss_vliq_new(ilev)*sp_dz(ilev)
         ENDIF
      ENDDO

      ! 界面处的总弥散系数 D = alpha_L * v + D_star
      DO ilev = 0, nlev
         D_intf(ilev) = alpha_L * ABS(q(ilev)) + D_star
            ! 然后显式分支
         IF (ilev == 0) THEN
            IF (ss_vliq_new(1) < ss_vliq_min) D_intf(0) = 0.0_r8
         ELSEIF (ilev == nlev) THEN
            IF (ss_vliq_new(nlev) < ss_vliq_min) D_intf(nlev) = 0.0_r8
         ELSE
            IF (ss_vliq_new(ilev) < ss_vliq_min .OR. ss_vliq_new(ilev+1) < ss_vliq_min) THEN
               D_intf(ilev) = 0.0_r8
            ENDIF
         ENDIF
         ion_flux_disp(ilev) = 0.0_r8
      ENDDO
      ...
      DO ilev = 1, nlev
         ...
         b(ilev) = 1.0_r8    
         c(ilev) = 0.0_r8  
         d(ilev) = ion_conc_vr(ilev)

         IF (.NOT. is_active_layer(ilev)) CYCLE

         IF (ilev == 1) THEN
            theta_intf = ss_vliq_new(1)
            dz_intf = sp_dz(1) / 2.0_r8
            b(1) = b(1) + dt * theta_intf * D_intf(0) / (ss_vliq_new(1) * dz_intf**2)
            d(1) = d(1) + dt * theta_intf * D_intf(0) * rainfall_ion_conc / (ss_vliq_new(1) * dz_intf**2)
         ELSE
            theta_intf = (ss_vliq_new(ilev-1) + ss_vliq_new(ilev)) / 2.0_r8
            dz_intf = (sp_dz(ilev-1) + sp_dz(ilev)) / 2.0_r8
            a(ilev) = -dt * theta_intf * D_intf(ilev-1) / (ss_vliq_new(ilev) * dz_intf**2)
            b(ilev) = b(ilev) - a(ilev)
         ENDIF

         IF (ilev == nlev) THEN
            theta_intf = ss_vliq_new(nlev)
            dz_intf = sp_dz(nlev) / 2.0_r8
            b(nlev) = b(nlev) + dt * theta_intf * D_intf(nlev) / (ss_vliq_new(nlev) * dz_intf**2)
            d(nlev) = d(nlev) + dt * theta_intf * D_intf(nlev) * groundwater_ion_conc / (ss_vliq_new(nlev) * dz_intf**2)
         ELSE
            theta_intf = (ss_vliq_new(ilev) + ss_vliq_new(ilev+1)) / 2.0_r8
            dz_intf = (sp_dz(ilev) + sp_dz(ilev+1)) / 2.0_r8
            c(ilev) = -dt * theta_intf * D_intf(ilev) / (ss_vliq_new(ilev) * dz_intf**2)
            b(ilev) = b(ilev) - c(ilev)
         ENDIF
        
      ENDDO

      CALL solve_tridiagonal(nlev, a, b, c, d, ion_conc_vr)

      DO ilev = 0, nlev
         IF ( D_intf(ilev) == 0.0_r8 ) THEN
            ion_flux_disp(ilev) = 0.0_r8
         ELSEIF (ilev == 0) THEN
            IF (is_active_layer(1)) THEN
               theta_intf = ss_vliq_new(1)
               ion_flux_disp(ilev) = -theta_intf * D_intf(0) * (ion_conc_vr(1) - rainfall_ion_conc) / (sp_dz(1)/2.0_r8)
            ENDIF
         ELSEIF (ilev == nlev) THEN
            IF (is_active_layer(nlev)) THEN
               theta_intf = ss_vliq_new(nlev)
               ion_flux_disp(ilev) = -theta_intf * D_intf(nlev) * (groundwater_ion_conc - ion_conc_vr(nlev)) / (sp_dz(nlev)/2.0_r8)
            ENDIF
         ELSE
            IF (is_active_layer(ilev) .OR. is_active_layer(ilev+1)) THEN
               theta_intf = (ss_vliq_new(ilev) + ss_vliq_new(ilev+1)) / 2.0_r8
               dz_intf = (sp_dz(ilev) + sp_dz(ilev+1)) / 2.0_r8
               ion_flux_disp(ilev) = -theta_intf * D_intf(ilev) * (ion_conc_vr(ilev+1) - ion_conc_vr(ilev)) / dz_intf
            ENDIF
         ENDIF
      ENDDO
      ...
      DO ilev = 1, nlev
         IF (is_active_layer(ilev)) THEN
            ion_mass_final = ion_mass_final + ion_conc_vr(ilev)*ss_vliq_new(ilev) * sp_dz(ilev)
         ENDIF
      ENDDO

      ion_flux_net = 0.0_r8
      IF (is_active_layer(1)) ion_flux_net = ion_flux_net + ion_flux_disp(0)
      IF (is_active_layer(nlev)) ion_flux_net = ion_flux_net - ion_flux_disp(nlev)
      
      ion_blc = (ion_mass_final - ion_mass_initial) - ion_flux_net * dt
      IF (ABS(ion_blc) > tol_ion) THEN
         write(*,*) 'Warning: HYDRO_dispersion balance error: ', ion_blc
      ENDIF

   END SUBROUTINE calculate_dispersion_implicit

   ! --- Thomas算法求解三对角方程组 ---
   SUBROUTINE solve_tridiagonal(n, a, b, c, d, x)
      IMPLICIT NONE
      integer,  intent(in)    :: n
      real(r8), intent(in)    :: a(n), b(n), c(n), d(n)
