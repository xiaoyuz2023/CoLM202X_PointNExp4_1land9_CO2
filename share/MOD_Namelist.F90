...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   ! ----- use soil acidification -----
   logical            :: DEF_USE_SOILACIDIFICATION = .false.
   logical            :: DEF_SOILACID_FACTOR_VC = .false.
   logical            :: DEF_SOILACID_FACTOR_ER = .false.
   logical            :: DEF_SOILACID_FACTOR_NITR = .false.

   logical            :: DEF_REST_SOILACIDFICATION = .false.

   ! ----- soil acidification tunable parameters (runtime-adjustable, no recompile needed) -----
   real(r8)           :: DEF_SOILACID_pH_opt        = 6.5_r8   ! optimal pH for Gaussian respiration function
   real(r8)           :: DEF_SOILACID_pH_sens       = 1.0_r8   ! pH sensitivity for Gaussian function (larger = less sensitive)
   real(r8)           :: DEF_SOILACID_neu_const      = 1.55e-9_r8 ! CaCO3 neutral dissolution rate [mol/cm2/s]
   real(r8)           :: DEF_SOILACID_acid_const     = 5.0e-4_r8  ! CaCO3 acid dissolution rate [mol/cm2/s]
   real(r8)           :: DEF_SOILACID_alk_const      = 3.3e-7_r8  ! CaCO3 alkali dissolution rate [mol/cm2/s]
   real(r8)           :: DEF_SOILACID_k_ref_co2      = 1.0e-3_r8  ! CO2 gas-liquid exchange rate at surface [s-1]
   real(r8)           :: DEF_SOILACID_z_ref_co2      = 0.5_r8   ! depth decay scale for CO2 exchange [m]
   real(r8)           :: DEF_SOILACID_exchange_calib = 0.0_r8  ! Damkohler limiter calibration [s*m2/gC]
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   type history_var_type
      ...
      logical :: smin_no3_vr                      = .true.
      logical :: smin_nh4_vr                      = .true.
      logical :: pH_vr                            = .true.
      logical :: pH_init_vr                       = .true.
      ! logical :: calcite_mass                     = .true.
      logical :: calcite_content                  = .true.
      logical :: h_from_N                         = .true.
      logical :: rate_cal                         = .true.
      logical :: rate_CO2                         = .true.
      logical :: rate_Bcex                        = .true.
      logical :: rate_Hex                         = .true.
      logical :: pCO2_soil                        = .true.
      logical :: E_Bc                             = .true.
      logical :: CEC                              = .true.
      logical :: h_conc_vr                        = .true.
      logical :: ca_conc_vr                       = .true.
      logical :: DIC_conc_vr                      = .true.
      logical :: ANC_conc_vr                      = .true.
      logical :: Bc_conc_vr                       = .true.
      logical :: Bct_conc_vr                      = .true.
      logical :: al3_conc_vr                        = .true.
...
CONTAINS
   ...
   namelist /nl_colm/                         &
      ...
      DEF_USE_SOILACIDIFICATION,              &
      DEF_SOILACID_FACTOR_VC,                 &
      DEF_SOILACID_FACTOR_ER,                 &
      DEF_SOILACID_FACTOR_NITR,               &
      DEF_REST_SOILACIDFICATION,              &

      DEF_SOILACID_pH_opt,                    &
      DEF_SOILACID_pH_sens,                   &
      DEF_SOILACID_neu_const,                 &
      DEF_SOILACID_acid_const,                &
      DEF_SOILACID_alk_const,                 &
      DEF_SOILACID_k_ref_co2,                 &
      DEF_SOILACID_z_ref_co2,                 &
      DEF_SOILACID_exchange_calib,            &
      ...
      DEF_FERT,                               & !add by Shiyu Zhang @ sysu 
...
! ----- BGC and CROP model related ------ Macros&Namelist conflicts and dependency management
         ...
         IF(DEF_USE_SOILACIDIFICATION)THEN
            DEF_USE_SOILACIDIFICATION = .false.
            write(*,*) '                  *****                  '
            write(*,*) 'Warning: Soil acidification is on when BGC is off.'
            write(*,*) 'DEF_USE_SOILACIDIFICATION is set to false automatically when BGC is turned off.'

         IF(DEF_SOILACID_FACTOR_VC)THEN
            DEF_SOILACID_FACTOR_VC = .false.
            write(*,*) '                  *****                  '
            write(*,*) 'Warning: Soil pH_factor in vm is on when BGC is off.'
            write(*,*) 'DEF_SOILACID_FACTOR_VC is set to false automatically when BGC is turned off.'

         IF(DEF_SOILACID_FACTOR_ER)THEN
            DEF_SOILACID_FACTOR_ER = .false.
            write(*,*) '                  *****                  '
            write(*,*) 'Warning: Soil pH_factor in soil_er is on when BGC is off.'
            write(*,*) 'DEF_SOILACID_FACTOR_ER is set to false automatically when BGC is turned off.'
        
            IF(DEF_SOILACID_FACTOR_NITR)THEN
            DEF_SOILACID_FACTOR_NITR = .false.
            write(*,*) '                  *****                  '
            write(*,*) 'Warning: Soil pH_factor in nitrification is on when BGC is off.'
            write(*,*) 'DEF_SOILACID_FACTOR_NITR is set to false automatically when BGC is turned off.'

         IF(DEF_REST_SOILACIDFICATION)THEN
            DEF_REST_SOILACIDFICATION = .false.
            write(*,*) '                  *****                  '
            write(*,*) 'Warning: The restart function during soil acidification is on when BGC is off.'
            write(*,*) 'DEF_REST_SOILACIDFICATION is set to false automatically when BGC is turned off.'
#endif
...
! ===============================================================
      ...
      CALL mpi_bcast (DEF_USE_SOILACIDIFICATION              ,1   ,mpi_logical   ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_FACTOR_VC                 ,1   ,mpi_logical   ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_FACTOR_ER                 ,1   ,mpi_logical   ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_FACTOR_NITR               ,1   ,mpi_logical   ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_REST_SOILACIDFICATION              ,1   ,mpi_logical   ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_pH_opt                   ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_pH_sens                  ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_neu_const                ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_acid_const               ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_alk_const                ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_k_ref_co2                ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_z_ref_co2                ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      CALL mpi_bcast (DEF_SOILACID_exchange_calib           ,1   ,mpi_real8     ,p_address_master ,p_comm_glb ,p_err)
      ...
      CALL mpi_bcast (DEF_FERT                               ,1   ,mpi_real      ,p_address_master ,p_comm_glb ,p_err)
   ...
   logical, intent(in) :: set_defaults
      ...
      CALL sync_hist_vars_one (DEF_hist_vars%er                 , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%soil_er            , set_defaults)
      ...
      CALL sync_hist_vars_one (DEF_hist_vars%smin_no3_vr , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%smin_nh4_vr , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%pH_vr       , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%pH_init_vr  , set_defaults)
      ! CALL sync_hist_vars_one (DEF_hist_vars%calcite_mass, set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%calcite_content, set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%h_from_N    , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%rate_cal    , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%rate_CO2    , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%rate_Bcex   , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%rate_Hex    , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%pCO2_soil   , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%E_Bc        , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%CEC         , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%h_conc_vr   , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%ca_conc_vr  , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%DIC_conc_vr , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%ANC_conc_vr , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%Bc_conc_vr  , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%Bct_conc_vr , set_defaults)
      CALL sync_hist_vars_one (DEF_hist_vars%al3_conc_vr   , set_defaults)
#endif
